...
export class HighByteSBCS extends sbcs {
  match(det: Context): Match | null {
...
export const caseFoldedByteMap = (pairs: number[][]) => {
  const byteMap = new Array<number>(256).fill(0x20);
  byteMap[0x27] = 0;

  for (let i = 0x41; i <= 0x5a; i++) byteMap[i] = i + 0x20;
  for (let i = 0x61; i <= 0x7a; i++) byteMap[i] = i;
...
export class windows_1257 extends sbcs {
  match(det: Context): Match | null {
    return det.inputBytes.some((byte) => byte >= 0x80)
      ? super.match(det)
      : null;
  ...
  byteMap() {
    ...
    for (let i = 0xc0; i <= 0xd6; i++) byteMap[i] = i + 0x20;
    for (let i = 0xd8; i <= 0xde; i++) byteMap[i] = i + 0x20;
    byteMap[0xdf] = 0xdf;
    for (let i = 0xe0; i <= 0xf6; i++) byteMap[i] = i;
    for (let i = 0xf8; i <= 0xfe; i++) byteMap[i] = i;

    return byteMap;
  }

  ngrams() {
    // Generated from original Estonian, Latvian, and Lithuanian prose samples.
    return [
      new NGramsPlusLang(
        'et',
        [
...
export class windows_1258 extends sbcs {
  match(det: Context): Match | null {
    return det.inputBytes.some((byte) => byte >= 0x80)
      ? super.match(det)
      : null;
  ...
  byteMap() {
    ...
    for (let i = 0xc0; i <= 0xcb; i++) byteMap[i] = i + 0x20;
    byteMap[0xcc] = 0xcc;
    for (let i = 0xcd; i <= 0xd1; i++) byteMap[i] = i + 0x20;
    byteMap[0xd2] = 0xd2;
    for (let i = 0xd3; i <= 0xd6; i++) byteMap[i] = i + 0x20;
    for (let i = 0xd8; i <= 0xdd; i++) byteMap[i] = i + 0x20;
    byteMap[0xde] = 0xde;
    byteMap[0xdf] = 0xdf;
    for (let i = 0xe0; i <= 0xf6; i++) byteMap[i] = i;
    for (let i = 0xf8; i <= 0xfd; i++) byteMap[i] = i;
    byteMap[0xff] = 0xff;
  ...
  ngrams() {
    // Generated from an original Vietnamese prose sample.
    return [
...
export class windows_874 extends sbcs {
  byteMap() {
    const byteMap = new Array<number>(256).fill(0x20);
    byteMap[0x27] = 0;
    ...
    for (let i = 0x41; i <= 0x5a; i++) byteMap[i] = i + 0x20;
    for (let i = 0x61; i <= 0x7a; i++) byteMap[i] = i;
    for (let i = 0xa1; i <= 0xda; i++) byteMap[i] = i;
    for (let i = 0xdf; i <= 0xfb; i++) byteMap[i] = i;
...
export class KOI8_U extends HighByteSBCS {
  byteMap() {
    return caseFoldedByteMap([
      [0xb3, 0xa3],
      [0xb4, 0xa4],
      [0xb6, 0xa6],
      [0xb7, 0xa7],
  ...
  ngrams() {
    // Generated from an original Ukrainian prose sample.
    return [
      0x20a420, 0x20ca20, 0x20cca6, 0x20cdc1, 0x20cdcf, 0x20cec1, 0x20cfc2,
      0x20cfc4, 0x20cfd3, 0x20d0c9, 0x20d0d2, 0x20d2cf, 0x20d4c1, 0x20d520,
      0x20d5cb, 0x20d7c9, 0x20ddcf, 0xa420c4, 0xa620ce, 0xa6d4c5, 0xa6d4c9,
      0xa7ced3, 0xc0d4d8, 0xc120cd, 0xc120dd, 0xc1a420, 0xc1a7ce, 0xc1c0d4,
      0xc1c7c1, 0xc1d4c9, 0xc1d4d5, 0xc1d7c9, 0xc1d7ce, 0xc4c1d7, 0xc5cbd3,
      0xc5cece, 0xc5d2c5, 0xc920ca, 0xc920ce, 0xc920cf, 0xc9ca20, 0xc9cbd5,
      0xc9d2c1, 0xca20d3, 0xcbc120, 0xcbc920, 0xcbd2c1, 0xcbd520, 0xcca6d4,
      0xcdcfd7, 0xced120, 0xcfc4ce, 0xcfd7c1, 0xd0d2c1, 0xd2c1a7, 0xd2cfda,
      0xd4a620, 0xd4c120, 0xd4c5d2, 0xd4c920, 0xd4d820, 0xd5cbd2, 0xd7a620,
      0xd7c120,
    ];
  }
...
export class IBM866 extends HighByteSBCS {
  byteMap() {
    return caseFoldedByteMap([
      [0x80, 0xa0],
      [0x81, 0xa1],
  ...
  ngrams() {
    // Generated from an original Russian prose sample.
    return [
      0x20a220, 0x20a2eb, 0x20a4a5, 0x20a4e0, 0x20a7a0, 0x20a820, 0x20a8e1,
      0x20aca0, 0x20ada0, 0x20aea1, 0x20afa8, 0x20afae, 0x20afe0, 0x20e0a0,
      0x20e1e2, 0x20e2a5, 0x20efa7, 0xa0a5e2, 0xa0a7a3, 0xa0e2a5, 0xa0e2ec,
      0xa0e6a8, 0xa0eee2, 0xa1e9a5, 0xa2aee0, 0xa2eba5, 0xa3aea2, 0xa4ada5,
      0xa4aeab, 0xa4e0e3, 0xa520a8, 0xa520af, 0xa520e1, 0xa5a2ad, 0xa5a4ad,
      0xa5ada8, 0xa5e1e2, 0xa5e220, 0xa7a3ae, 0xa7ebaa, 0xa820a8, 0xa820ad,
      0xa820ae, 0xa820af, 0xa8a0ab, 0xa8a520, 0xa8a820, 0xa8a920, 0xa920a8,
      0xaba820, 0xaca0e2, 0xada5a2, 0xada8a5, 0xada8a8, 0xaea2ae, 0xaeac20,
      0xe0a0a7, 0xe1ef20, 0xe2ec20, 0xeba520, 0xeba920, 0xebac20, 0xeee220,
      0xefa7eb,
    ];
  }
...
export class IBM855 extends HighByteSBCS {
  byteMap() {
    return caseFoldedByteMap([
      [0x81, 0x80],
      [0x83, 0x82],
  ...
  ngrams() {
    // Generated from original Bulgarian, Russian, and Serbian prose samples.
    return [
      new NGramsPlusLang(
        'bg',
        [
          0x20a6a0, 0x20a6a8, 0x20a8f3, 0x20b720, 0x20b7f3, 0x20d4a0, 0x20d6a2,
          0x20d8b7, 0x20d8d6, 0x20d8e1, 0x20e1a0, 0x20e3a8, 0x20e5a8, 0x20e7fb,
          0x20eb20, 0x9ed0ac, 0x9ee520, 0xa020a6, 0xa020b7, 0xa020d4, 0xa020d8,
          0xa020e3, 0xa0e520, 0xa0e5a0, 0xa0f3a2, 0xa29ed0, 0xa2b7e1, 0xa2f9a8,
          0xa6a020, 0xa820e1, 0xa820e7, 0xa8a6d4, 0xa8c6e3, 0xa8d4b7, 0xa8e3e5,
          0xa8e5d6, 0xa8f3b7, 0xaca020, 0xaca0e1, 0xb720a6, 0xb720b7, 0xb720e3,
          0xb7c620, 0xb7de20, 0xb7dee5, 0xb7e5a8, 0xd0aca0, 0xd4a020, 0xd4b7de,
          0xd620e3, 0xd6ebb7, 0xd8e1a0, 0xdee520, 0xe1a0e5, 0xe1a0f3, 0xe3a820,
          0xe3e5eb, 0xe520b7, 0xe5a020, 0xe5a0e5, 0xe5a820, 0xe5d620, 0xebd6e5,
          0xf3b7c6,
        ],
      ),
      new NGramsPlusLang(
        'ru',
        [
          0x20a6a8, 0x20a6e1, 0x20b720, 0x20b7e3, 0x20d2a0, 0x20d4a0, 0x20d6a2,
          0x20d8b7, 0x20d8d6, 0x20d8e1, 0x20def3, 0x20e1a0, 0x20e3e5, 0x20e5a8,
          0x20eb20, 0x20ebf1, 0x20f3a0, 0x9ce520, 0xa09ce5, 0xa0a4b7, 0xa0a8e5,
          0xa0e5a8, 0xa0e5ed, 0xa0f3ac, 0xa2f9a8, 0xa6d4a8, 0xa6d6d0, 0xa6e1e7,
          0xa820b7, 0xa820d8, 0xa820e3, 0xa8a6d4, 0xa8d4b7, 0xa8e3e5, 0xa8e520,
          0xa8ebd4, 0xacd6eb, 0xb720b7, 0xb720d4, 0xb720d6, 0xb720d8, 0xb7a0d0,
          0xb7a820, 0xb7b720, 0xb7bd20, 0xbd20b7, 0xd0b720, 0xd2a0e5, 0xd4a8eb,
          0xd4b7a8, 0xd4b7b7, 0xd4dee5, 0xd4f1bd, 0xd4f1d2, 0xd6d220, 0xd6ebd6,
          0xdef3f1, 0xe1a0f3, 0xe3de20, 0xe5ed20, 0xf1a820, 0xf1bd20, 0xf1d220,
          0xf3f1c6,
        ],
      ),
      new NGramsPlusLang(
        'sr',
        [
          0x208ea0, 0x208ea8, 0x20a2d6, 0x20a6a0, 0x20b720, 0x20b7f3, 0x20c692,
          0x20c6d6, 0x20d4a0, 0x20d8b7, 0x20d8d6, 0x20d8e1, 0x20e1a0, 0x20e1a8,
          0x20e3a8, 0x20e3e5, 0x20e3eb, 0x20e5a8, 0x20e720, 0x20eba8, 0x20f5c6,
          0x20fbb7, 0x8ea8f3, 0x8ee720, 0xa020a6, 0xa0c6d6, 0xa0ebd4, 0xa0f3e7,
          0xa6a020, 0xa6d4a8, 0xa820d8, 0xa820e1, 0xa820e3, 0xa820eb, 0xa8c6e3,
          0xa8ebd4, 0xa8f3b7, 0xacd6eb, 0xb7208e, 0xb720a6, 0xb720b7, 0xb720d4,
          0xb720e3, 0xb720e5, 0xb720e7, 0xb78ea0, 0xb7c620, 0xb7d2a0, 0xb7d4a8,
          0xb7e3e5, 0xb7e5a0, 0xb7f5e7, 0xc692b7, 0xc6d6a6, 0xc6d6d0, 0xd0b720,
          0xd4d620, 0xd6d220, 0xe1a0f3, 0xe3a820, 0xe3e5b7, 0xe720b7, 0xe720d8,
          0xebd4d6,
        ],
      ),
    ];
...
export class x_mac_cyrillic extends HighByteSBCS {
  byteMap() {
    return caseFoldedByteMap([
      [0x80, 0xe0],
      [0x81, 0xe1],
  ...
  ngrams() {
    // Generated from original Russian and Ukrainian prose samples.
    return [
      new NGramsPlusLang(
        'ru',
        [
          0x20dfe7, 0x20e220, 0x20e2fb, 0x20e4e5, 0x20e4f0, 0x20e7e0, 0x20e820,
          0x20e8f1, 0x20ece0, 0x20ede0, 0x20eee1, 0x20efe8, 0x20efee, 0x20eff0,
          0x20f0e0, 0x20f1f2, 0x20f2e5, 0xdf20e2, 0xdfe7fb, 0xe0e5f2, 0xe0e7e3,
          0xe0f2e5, 0xe0f2fc, 0xe0f6e8, 0xe0fef2, 0xe1f9e5, 0xe2eef0, 0xe2fbe5,
          0xe3eee2, 0xe4ede5, 0xe4eeeb, 0xe4f0f3, 0xe520e8, 0xe520ef, 0xe520f1,
          0xe5e2ed, 0xe5e4ed, 0xe5ede8, 0xe5f1f2, 0xe5f220, 0xe7e3ee, 0xe7fbea,
          0xe820e8, 0xe820ed, 0xe820ee, 0xe820ef, 0xe8e0eb, 0xe8e520, 0xe8e820,
          0xe8e920, 0xe920e8, 0xebe820, 0xece0f2, 0xeddff2, 0xede8e5, 0xeee2ee,
          0xeeec20, 0xf0e0e7, 0xf1df20, 0xf2fc20, 0xfbe520, 0xfbe920, 0xfbec20,
          0xfef220,
        ],
      ),
      new NGramsPlusLang(
        'uk',
        [
          0x20b920, 0x20e2e8, 0x20e920, 0x20ebb4, 0x20ece0, 0x20ecee, 0x20ede0,
          0x20eee1, 0x20eee4, 0x20eef1, 0x20efe8, 0x20eff0, 0x20f0ee, 0x20f2e0,
          0x20f320, 0x20f3ea, 0x20f9ee, 0xb420ed, 0xb4f2e5, 0xb4f2e8, 0xb920e4,
          0xbbedf1, 0xdf20e2, 0xe020ec, 0xe020f9, 0xe0b920, 0xe0bbed, 0xe0e2e8,
          0xe0e2ed, 0xe0e3e0, 0xe0f2e8, 0xe0f2f3, 0xe0fef2, 0xe2b420, 0xe2e020,
          0xe2e8ed, 0xe4e0e2, 0xe5eaf1, 0xe5eded, 0xe5f0e5, 0xe7f3ec, 0xe820e9,
          0xe820ed, 0xe820ee, 0xe8e920, 0xe8eaf3, 0xe8f0e0, 0xe920f1, 0xeae820,
          0xeaf0e0, 0xeceee2, 0xeddf20, 0xeee2e0, 0xeee4ed, 0xeff0e0, 0xf0e0bb,
          0xf0eee7, 0xf2b420, 0xf2e020, 0xf2e5f0, 0xf2e820, 0xf2fc20, 0xf3eaf0,
          0xfef2fc,
        ],
      ),
    ];
...
export class macintosh extends HighByteSBCS {
  match(det: Context): Match | null {
    const result = super.match(det);
    if (
      result &&
      det.inputBytes.some((byte) =>
        [0x81, 0x8d, 0x8f, 0x90, 0x9d].includes(byte),
      )
    ) {
  ...
  ngrams() {
    // Generated from original English, French, German, and Spanish prose samples.
    return [
      new NGramsPlusLang(
        'en',
        [
          0x206163, 0x20616e, 0x206265, 0x20636f, 0x20646f, 0x206869, 0x206964,
          0x206f72, 0x207065, 0x207072, 0x207075, 0x207175, 0x207265, 0x20736f,
          0x207374, 0x207379, 0x207468, 0x20756e, 0x207573, 0x207765, 0x207772,
          0x616363, 0x616c20, 0x616e64, 0x617469, 0x626c69, 0x636f64, 0x636f6d,
          0x636f6e, 0x63756d, 0x642061, 0x646572, 0x646f63, 0x652063, 0x652068,
          0x652073, 0x652074, 0x652077, 0x656420, 0x656c79, 0x656e74, 0x657220,
          0x657265, 0x657273, 0x657320, 0x657365, 0x666f72, 0x696465, 0x696e67,
          0x6c7920, 0x6e6420, 0x6e6720, 0x6f6e20, 0x726520, 0x727320, 0x732061,
          0x736520, 0x73746f, 0x746572, 0x746865, 0x74696e, 0x746f72, 0x747320,
          0x757365,
        ],
      ),
      new NGramsPlusLang(
        'fr',
        [
          0x206176, 0x20636f, 0x206461, 0x206465, 0x20648e, 0x20656e, 0x206573,
          0x206574, 0x206a6f, 0x206c61, 0x206c65, 0x206e75, 0x207072, 0x20728e,
          0x207472, 0x20756e, 0x208e63, 0x208e6c, 0x208e74, 0x616972, 0x616e63,
          0x616e73, 0x616e8d, 0x619474, 0x636869, 0x636f6e, 0x63728e, 0x646520,
          0x646573, 0x648e65, 0x648e6a, 0x652063, 0x652065, 0x652066, 0x65206c,
          0x65208e, 0x656e74, 0x656e90, 0x657320, 0x657420, 0x657572, 0x69648e,
          0x696f6e, 0x69738e, 0x6a8820, 0x6c6520, 0x6c6573, 0x6c8f76, 0x6e7320,
          0x6e7420, 0x6f6e20, 0x72616e, 0x726520, 0x727320, 0x732061, 0x732063,
          0x732064, 0x73206c, 0x73696f, 0x742064, 0x747265, 0x747320, 0x757220,
          0x757820,
        ],
      ),
      new NGramsPlusLang(
        'de',
        [
          0x206175, 0x206265, 0x20629f, 0x206465, 0x20646f, 0x206572, 0x206765,
          0x20696e, 0x206b6c, 0x207370, 0x20756e, 0x207665, 0x207769, 0x207a65,
          0x207a75, 0x209a66, 0x616265, 0x616368, 0x629f63, 0x636820, 0x636865,
          0x63686b, 0x64656e, 0x646965, 0x646f6b, 0x652062, 0x652064, 0x652067,
          0x652073, 0x652075, 0x652077, 0x656974, 0x656e20, 0x656e73, 0x656e74,
          0x657220, 0x657265, 0x65726e, 0x657273, 0x657275, 0x66656e, 0x666665,
          0x67656e, 0x676c69, 0x686520, 0x68656e, 0x696368, 0x696520, 0x696e20,
          0x6c6520, 0x6c6963, 0x6d656e, 0x6e2077, 0x6e6420, 0x726520, 0x727374,
          0x736368, 0x737072, 0x746520, 0x746572, 0x746c69, 0x756e64, 0x756e67,
          0x766572,
        ],
      ),
      new NGramsPlusLang(
        'es',
        [
          0x206120, 0x206361, 0x20636c, 0x20636f, 0x206465, 0x20656c, 0x20656e,
          0x206573, 0x206c61, 0x206c65, 0x206c69, 0x206f72, 0x207061, 0x207065,
          0x207072, 0x207265, 0x20756e, 0x207920, 0x612061, 0x612063, 0x612065,
          0x616369, 0x616461, 0x61646f, 0x616c20, 0x616e74, 0x617261, 0x617320,
          0x61966f, 0x636163, 0x636164, 0x636997, 0x636f64, 0x636f6d, 0x636f6e,
          0x636f73, 0x646120, 0x646520, 0x646f20, 0x652063, 0x656c20, 0x656e20,
          0x656e74, 0x657220, 0x65726f, 0x657320, 0x666963, 0x696361, 0x69636f,
          0x696669, 0x696769, 0x696c69, 0x69976e, 0x6c2073, 0x6c6120, 0x6e746f,
          0x6f206c, 0x6f6e20, 0x6f7320, 0x707265, 0x732061, 0x732070, 0x746f73,
          0x976e20,
        ],
      ),
    ];
